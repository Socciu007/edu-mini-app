...
describe('useSurveySubmit', () => {
  ...
  it('on valid form, calls API, returns response, opens success snackbar', async () => {
    ...
    expect(openSnackbar).toHaveBeenCalledWith(expect.objectContaining({ text: expect.any(String), type: 'success' }))
  })
  ...
  it('on API error, opens error snackbar and returns null', async () => {
    ...
    expect(returned).toBeNull()
    expect(openSnackbar).toHaveBeenCalledWith(expect.objectContaining({ text: expect.any(String), type: 'error' }))
  })
})
